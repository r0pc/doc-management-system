...
| **#25** | Access gated on clearance rank and department subtree | `app/domain/policy.py:can_access` enforces clearance $\ge$ rank AND *any* assigned department $\in$ visible subtrees. Membership lives in `document_departments` (migration `0007`), and the SQL half is a single shared clause in `app/db/visibility.py:department_clause` — previously hand-written in five query sites, where one missed edit would have widened access silently. `documents.department_id` records the OWNING department only and is never consulted for access. |
