...
| **11** | **Authentication Front Door.** Login page at `/login` with email + password, replacing the auto-login that silently granted every visitor a clearance-4 Security Admin session on mount. `RequireAuth` guard redirects every route to `/login` and restores the requested URL afterwards (`?next=`, constrained against open redirect). Five demo accounts, one per role, spanning all four security levels, defined once in `app/security/demo_accounts.py` and published by `GET /v1/auth/demo-accounts` so the login page can list and prefill them. `POST /v1/auth/login` is mounted only under `env=dev` and re-checks per handler; rejection is byte-identical and equal-time for an unknown email and a wrong password. Fixed a latent identity bug: the token subject is now the `oidc_sub` seeded by migration `0003`, so signing in binds to the seeded `users` row instead of provisioning a duplicate (`dev-admin_t1` vs `dev-admin`) — the dev DB held 5 such orphans. Client-side token-expiry sweep drops a dead session rather than rendering a shell that 401s. | ✅ Complete |
...
Frontend unit tests        -> vitest run: 132 passed across 11 test suites (hermetic, e2e excluded)
Frontend E2E test suite    -> playwright test: 50 passed across 9 specs (admin-classifier, admin-detectors, bulk-upload, deployment, list-controls, login, security, smoke, upload-journey)
Backend hermetic tests     -> pytest -q: 827 passed, 3 skipped, 29 deselected, 5 warnings in 33.50s
Backend typecheck          -> mypy app: Success: no issues found in 70 source files (strict)
Backend linting            -> ruff check .: All checks passed!
Backend formatting         -> ruff format --check .: 175 files already formatted
