...
def test_status_filter_reaches_the_query(client, captured_page_args) -> None:
    client.get("/v1/documents?status=failed")
    assert captured_page_args["status"] == "failed"


def test_level_filter_reaches_the_query(client, captured_page_args) -> None:
    client.get("/v1/documents?security_level=confidential")
    assert captured_page_args["level"] == "confidential"


def test_absent_filters_are_none(client, captured_page_args) -> None:
...
def test_unknown_status_is_rejected_not_ignored(client, bad: str) -> None:
